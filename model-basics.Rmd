...
The goal of a model is to provide a simple low-dimensional summary of a data set. In the context of this book we're going to use models to partition data into patterns and residuals. Strong patterns will hide subtler trends, so we'll use models to help peel back layers of structure as we explore a data set.  

However, before we can start using models on interesting, real, data sets, you need to understand the basics of how models work. For that reason, this chapter of the book is unique because it uses only simulated data sets. These data sets are very simple, and not at all interesting, but they will help you understand the essence of modelling before you apply the same techniques to real data in the next chapter.  
...
Lets take a look at the simulated data set `sim1`, included with the modelr package. It contains two continuous variables, `x` and `y`. Let's plot them to see how they're related:  
...
#### Exercise 1  
1.  One downside of the linear model is that it is sensitive to unusual values because the distance incorporates a squared term. Fit a linear model to the simulated data below, and visualize the results. Rerun a few times to generate different simulated data sets. What do you notice about the model?  
...
*scatter point plot + abline*    
```{r}
ggplot() + 
    geom_point(mapping = aes(x, y), data = sim1a) +
    geom_abline(intercept = coef(sim1a_model)[1], slope = coef(sim1a_model)[2])
```
...
*measure distance with mean-absolute distance*  
```{r}
measure_distance <- function(mod, data) {
  diff <- data$y - model1(mod, data)
  mean(abs(diff))
...
#### Exercise 3  
3.  One challenge with performing numerical optimization is that it's only guaranteed to find one local optimum. What's the problem with optimizing a three parameter model like this?  

*Optimize a three parameter model*  
```{r}
model1 <- function(a, data) {
  a[1] + data$x * a[2] + a[3]
}
...
ggplot(sim1a, aes(x, y)) + 
  geom_point(size = 2, colour = "grey30") + 
  geom_abline(intercept = best$par[1] + best$par[3], slope = best$par[2])
```

**Same as the 2 parmeter model**
...
### 3.1 Predictions  
To visualize the predictions from a model, we start by generating an evenly spaced grid of values that covers the region where our data lies. The easiest way to do that is to use `modelr::data_grid()`. Its first argument is a data frame, and for each subsequent argument it finds the unique variables and then generates all combinations:  
...
(You can also use this function to add predictions to your original data set.)  
...
ggplot(sim1, aes(x)) +
  geom_point(aes(y = y)) +
  geom_point(aes(x = x, y = pred), data = grid, colour = "blue", size = 2) +
...
We add residuals to the data with `add_residuals()`, which works much like `add_predictions()`. Note, however, that we use the original data set, not a manufactured grid. This is because to compute residuals we need actual y values.  
```{r}
sim1 <- sim1 %>% 
...
This looks like random noise, suggesting that our model has done a good job of capturing the patterns in the data set.  
...
*Loess model and smooth curve*  
```{r}
sim1_loess_model <- loess(y ~ x, data = sim1)
...
grid <- grid %>% 
  add_predictions(sim1_loess_model)
grid
```
...
```{r}
sim1 <- sim1 %>% 
  add_residuals(sim1_loess_model,var = "resid_loess")
sim1
```
...
grid <- grid %>% 
  spread_predictions(sim1_loess_model)
grid
```
...
grid <- grid %>% 
  gather_predictions(sim1_loess_model)
grid
```
...
*Visualize residual distributions with `geom_freqpoly()`*
```{r}
ggplot(sim1, aes(resid_loess)) + 
  geom_freqpoly(binwidth = 0.5)
```
...
Fortunately, however, if you focus on visualizing predictions you don't need to worry about the exact parameterization. Let's look at some data and models to make that concrete. Here's the sim2 data set from `modelr`:  
...
### 4.2 Interactions (continuous and categorical)  
...
### 4.3 Interactions (two continuous)  
...
First, let's visualize the data with 3D plot using `plotly`.
```{r}
library(plotly)
plot_ly(data = sim4, 
        x = ~ x1, 
...
Predict based on the model without the intercept **Use `- 1` to remove intercept.**  

```{r}
mod_sim2_no_intercept <- lm(y ~ x -1, data = sim2)
mod_sim2 <- lm(y ~ x, data = sim2)
grid <- sim2 %>% 
  data_grid(x) %>%
  add_predictions(mod_sim2_no_intercept)
grid
```

Compare the models with and without the intercept  

```{r}
mod_sim2
mod_sim2_no_intercept
```

Compare the model equations.  

Compare `intercept; slope: xb, xc, xd; residual standard error` and `intercept; slope: xa, xb, xc, xd; residual standard error`of the two models.  \
**equation:**  
$$y_{i} = \beta_{0} +\beta_{1}x_{i} + \epsilon_{i}$$  

```{r}
summary(mod_sim2)
summary(mod_sim2_no_intercept)
```
...
*Model matrixes for sim3*  
```{r}
model_matrix(mod1_sim3, data = sim3)
model_matrix(mod2_sim3, data = sim3)
```

*Models for sim4 and add predictions*   
```{r}
...
grid <- sim4 %>% 
  ...
  gather_predictions(mod1_sim4, mod2_sim4)
grid
```
...
*Summary of the models for sim3 showed coefficients and residuals*  
```{r}
summary(mod1_sim3)
summary(mod2_sim3)
```
**mod1_sim3 equation:**  
$$y = 1.87167 - 0.19674 + 2.88781x_{2}b + 4.80574x_{2}c + 2.35959x_{2}d$$  


**mod2_sim3 equation:**  
$$y = 1.30124 - 0.09302 + 7.06938x_{2}b + 4.43090x_{2}c + 0.83455x_{2}d - 0.76029x_{1}x_{2}b + 0.06815x_{1}x_{2}c + 0.27728x_{1}x_{2}d$$  

*Summary of the models for sim4 showed coefficients and residuals*  
```{r}
summary(mod1_sim4)
summary(mod2_sim4)
```

**Equation of mod1_sim4:**   
$$y = 0.03546 + 1.82167x_{1} - 2.78252x_{2}$$  

**Equation of mod2_sim4:**  
$$y = 0.03546 + 1.82167x_{1} - 2.78252x_{2} + 0.95228x_{1}x_{2}$$  

##### Exercise 4  
4. For sim4, which of mod1 and mod2 is better? I think mod2 does a slightly better job at removing patterns, but it’s pretty subtle. Can you come up with a plot to support my claim?    

*Add predictions by the two models*  
```{r}
grid <- sim4 %>%
  data_grid(
    x1, x2
...
*Add residuals by the two models*  
```{r}
sim4 <- sim4 %>%
  gather_residuals(mod1_sim4, mod2_sim4)
sim4
```

*Visualize the residuals with 3D plot*
```{r}
plot_ly(data = sim4, x = ~ x1, y = ~ x2, z = ~ resid, type = "scatter3d", mode = "markers", size = 1, colour = ~ model) 
```

*Visualize the residuals with `geom_freqpoly()`*
```{r}
...
This chapter has focused exclusively on the class of linear models, which assume a relationship of the form `y = a_1 * x1 + a_2 * x2 + ... + a_n * xn`. Linear models additionally assume that the residuals have a normal distribution, which we haven't talked about. There are a large set of model classes that extend the linear model in various interesting ways. Some of them are:  
