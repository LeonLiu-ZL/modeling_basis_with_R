...
In the previous chapter you learned how linear models work, and learned some basic tools for understanding what a model is telling you about your data. The previous chapter focused on simulated data sets. This chapter will focus on real data, showing you how you can progressively build up a model to aid your understanding of the data.
...
We can make it easier to see how the other attributes of a diamond affect its relative `price` by fitting a model to separate out the effect of `carat`. But first, lets make a couple of tweaks to the diamonds data set to make it easier to work with:
...
That tells us something interesting about our data. If we believe our model, then the large diamonds are much cheaper than expected. This is probably because no diamond in this data set costs more than \$19,000.
...
If the model needs variables that you haven't explicitly supplied, [`data_grid()`](https://modelr.tidyverse.org/reference/data_grid.html) will automatically fill them in with "typical" value. For continuous variables, it uses the median, and categorical variables it uses the most common value (or values, if there's a tie).
```{r}
diamonds2 <- diamonds2 %>% 
...
Let's work through a similar process for a data set that seems even simpler at first glance: the number of flights that leave NYC per day. This is a really small data set --- only 365 rows and 2 columns --- and we're not going to end up with a fully realized model, but as you'll see, the
steps along the way will help us better understand the data. Let's get started by counting the number of flights per day and visualizing it with ggplot2.
```{r}
daily <- flights %>% 
...
```{r}
daily <- daily %>% 
  mutate(wday = wday(date, label = TRUE, locale = "en_US"))
...
-   There seems to be some smoother long term trend over the course of a year. We can highlight that trend with [`geom_smooth()`](https://ggplot2.tidyverse.org/reference/geom_smooth.html):
```{r}
daily %>% 
  ggplot(aes(date, resid)) + 
  geom_ref_line(h = 0) + 
...
Our model is finding the *mean* effect, but we have a lot of big outliers, so mean tends to be far away from the typical value. We can alleviate this problem by using a model that is robust to the effect of outliers: [`MASS::rlm()`](https://rdrr.io/pkg/MASS/man/rlm.html). This greatly reduces the impact of the outliers on our estimates, and gives a model that does a good job of removing the day of week pattern:
```{r}
...
1.  Use your Google sleuthing skills to brainstorm why there were fewer than expected flights on Jan 20, May 26, and Sep 1. (Hint: they all have the same explanation.) How would these days generalist to another year?
...
> Nov 30: Saturday of Thanksgiving week /
Dec 1: Sunday of Thanksgiving week /
Dec 28: Saturday of Christmas week.
...
daily <- flights %>% 
  ...
  mutate(wday = if_else(wday == "Sat", paste(wday, term, sep = "-"), wday))

model_flight <- lm(n ~ wday, data = daily)

daily %>%
  add_residuals(model_flight) %>%
  ggplot() +
...
```{r}
holidays <- ymd(tis::holidays(2013))[c(2,4,5,6,9,10)]
...
for (i in 1:length(holidays)) {
  week_day = wday(holidays[i], label = T)
  if (week_day == "Mon") {
    weekend = holidays[i] - 2 
  } else if (week_day == "Thu") {
    weekend = holidays[i]  + 2
  } else if (week_day == "Wed"){
    weekend = holidays[i] + 3
  }
  daily <- daily %>%
    mutate(term = if_else(date == weekend, paste("holiday", term, sep = "-"), term))
} 
...
for (i in 1:length(holidays)) {
  week_day = wday(holidays[i], label = T)
  if (week_day == "Mon") {
    holiday_weekend = c(holiday_weekend, holidays[i] - 2)
  } else if (week_day == "Thu") {
    holiday_weekend = c(holiday_weekend, holidays[i]  + 2)
  } else if (week_day == "Wed"){
    holiday_weekend = c(holiday_weekend, holidays[i]  + 3)
  }
} 
...
daily <- flights %>% 
  ...
  mutate(wday = as.character(wday)) %>%
  mutate(term = as.character(term)) %>%
  mutate(is_holiday_weekend = if_else(date %in% holiday_weekend, "Yes", "No"))

model_flight <- lm(n ~ wday + term + is_holiday_weekend, data = daily)
...
model_flight <- lm(n ~ wday * month, data = daily)
...
model_flight <- lm(n ~ wday + ns(date, 5), data = daily)
...
```{r}
sunday_flights <- flights %>% 
  mutate(date = make_date(year, month, day)) %>% 
  group_by(date) %>% 
  mutate(wday = wday(date, label = TRUE, locale = "en_US")) %>%
  filter(wday == "Sun") %>%
  mutate(dep_hour = substr(str_pad(dep_time, 4, "left", "0"), 1, 2)) %>%
  mutate(dep_minute = substr(str_pad(dep_time, 4, "left", "0"), 3, 4)) %>%
  mutate(departure_time = 60 * as.numeric(dep_hour) + as.numeric(dep_minute))

ggplot(sunday_flights) + 
  geom_histogram(aes(departure_time), bins = 292) + 
  geom_vline(xintercept = 720, color = "red")

ggplot(sunday_flights) + 
  geom_histogram(aes(distance), bins = 100)
```
...
daily <- flights %>% 
  ...
  group_by(date) %>% 
  summarise(n = n()) %>%
  mutate(wday = wday(date, label = TRUE, locale = "en_US", week_start = 1)) %>%
  mutate(term = term(date)) 
