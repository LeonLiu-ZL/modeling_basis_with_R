...
1.  In [list-columns](https://r4ds.had.co.nz/many-models.html#list-columns-1), you'll learn more about the list-column data structure, and why it's valid to put lists in data frames.  

2.  In [creating list-columns](https://r4ds.had.co.nz/many-models.html#creating-list-columns), you'll learn the three main ways in which you'll create list-columns.  

3.  In [simplifying list-columns](https://r4ds.had.co.nz/many-models.html#simplifying-list-columns) you'll learn how to convert list-columns back to regular atomic vectors (or sets of atomic vectors) so you can work with them more easily.

4.  In [making tidy data with broom](https://r4ds.had.co.nz/many-models.html#making-tidy-data-with-broom), you'll learn about the full set of tools provided by broom, and see how they can be applied to other types of data structure.  
...
And we want to apply it to every data frame. The data frames are in a list, so we can use [`purrr::map()`](https://purrr.tidyverse.org/reference/map.html) to apply `country_model` to each element:  
```{r}
...
In other words, instead of creating a new object in the global environment, we're going to create a new variable in the `by_country` data frame. That's a job for [`dplyr::mutate()`](https://dplyr.tidyverse.org/reference/mutate.html):  
```{r}
...
Previously we computed the residuals of a single model with a single dataset. Now we have 142 data frames and 142 models. To compute the residuals, we need to call [`add_residuals()`](https://modelr.tidyverse.org/reference/add_residuals.html) with each model-data pair:  
```{r}
by_country <- by_country %>% 
  mutate(
...
Instead of looking at the residuals from the model, we could look at some general measurements of model quality. You learned how to compute some specific measures in the previous chapter. Here we'll show a different approach using the broom package. The broom package provides a general set of functions to turn models into tidy data. Here we'll use [`broom::glance()`](https://generics.r-lib.org/reference/glance.html) to extract some model quality metrics. If we apply it to a model, we get a data frame with a single row:  
```{r}
    broom::glance(nz_mod)
```
...
This isn't quite the output we want, because it still includes all the list columns (**This is an old version dplyr. Now `.drop=TRUE` is not working any more. All list-columns are now preserved; If there are any that you don't want in the output use select() to remove them prior to unnesting**). This is default behavior when `unnest()` works on single row data frames. To suppress these columns we use `.drop = TRUE`:  
```{r}
glance <- by_country %>% 
  mutate(glance = map(model, broom::glance)) %>% 
...
The worst models all appear to be in Africa. Let's double check that with a plot. Here we have a relatively small number of observations and a discrete variable, so [`geom_jitter()`](https://ggplot2.tidyverse.org/reference/geom_jitter.html) is effective:
```{r}
glance %>% 
...
*Model function*  
```{r}
quadratic_mdodel <- function(df) {
    lm(lifeExp ~ year_trans + year2, df)
}
```

*Data transform*  
```{r}
data_quadratic_mdodel <- gapminder %>% 
    mutate(year_trans = year - mean(year)) %>%
    mutate(year2 = year ^ 2) %>%
    group_by(country, continent) %>%
    nest() %>% 
    mutate(quad_model = map(data, quadratic_mdodel)) %>%
    mutate(resids = map2(data, quad_model, add_residuals))
```

*Visualize year-life expectation curves by continents*  
```{r}
data_quadratic_mdodel %>%
    unnest(resids) %>%
    ggplot(aes(year, resid)) +
        geom_line(aes(group = country), alpha= 1/3) +
        facet_wrap(~ continent) +
...
```{r}
library(ggbeeswarm)
ggplot(glance, aes(continent, r.squared, color = continent)) +
    geom_beeswarm() +
    theme_classic() +
    guides(color = "none")
```
...
ggplot(bad_fit, aes(year, lifeExp, colour = country)) +
    geom_line() + 
    theme_classic() + 
...
1.  You create the list-column using one of `nest()`, `summarise()` + [`list()`](https://rdrr.io/r/base/list.html), or `mutate()` + a map function, as described in [Creating list-columns](https://r4ds.had.co.nz/many-models.html#creating-list-columns).
...
3.  You simplify the list-column back down to a data frame or atomic vector, as described in [Simplifying list-columns](https://r4ds.had.co.nz/many-models.html#simplifying-list-columns).
...
1.  With [`tidyr::nest()`](https://tidyr.tidyverse.org/reference/nest.html) to convert a grouped data frame into a nested data frame where you have list-column of data frames.
...
Alternatively, you might create them from a named list, using [`tibble::enframe()`](https://tibble.tidyverse.org/reference/enframe.html).
...
### 4.2 From vectorised functions

Some useful functions take an atomic vector and return a list. For example, in [strings](https://r4ds.had.co.nz/strings.html#strings) you learned about `stringr::str_split()` which takes a character vector and returns a list of character vectors. If you use that inside mutate, you'll get a list-column:
```{r}
df <- tribble(
  ~x1,
  "a,b,c", 
...
(If you find yourself using this pattern a lot, make sure to check out [`tidyr::separate_rows()`](https://tidyr.tidyverse.org/reference/separate_rows.html) which is a wrapper around this common pattern).

Another example of this pattern is using the `map()`, `map2()`, `pmap()` from purrr. For example, we could take the final example from [Invoking different functions](https://r4ds.had.co.nz/iteration.html#invoking-different-functions) and rewrite it to use `mutate()`:
```{r}
sim <- tribble(
  ~f,      ~params,
...
### 4.3 From multivalued summaries

One restriction of `summarise()` is that it only works with summary functions that return a single value. That means that you can't use it with functions like [`quantile()`](https://rdrr.io/r/stats/quantile.html) that return a vector of arbitrary length:
```{r}
mtcars %>% 
  group_by(cyl) %>% 
...
mtcars %>% 
  ...
  unnest(c(p, q))
```
### 4.4 From a named list

Data frames with list-columns provide a solution to a common problem: what do you do if you want to iterate over both the contents of a list and its elements? Instead of trying to jam everything into one object, it's often easier to make a data frame: one column can contain the elements, and one column can contain the list. An easy way to create such a data frame from a list is [`tibble::enframe()`](https://tibble.tidyverse.org/reference/enframe.html).
```{r}
x <- list(
  a = 1:5,
  b = 3:4, 
...
The advantage of this structure is that it generalizes in a straightforward way - names are useful if you have character vector of metadata, but don't help if you have other types of data, or multiple vectors.
...
2.  Brainstorm useful summary functions that, like [`quantile()`](https://rdrr.io/r/stats/quantile.html), return multiple values.

3.  What's missing in the following data frame? How does [`quantile()`](https://rdrr.io/r/stats/quantile.html) return that missing piece? Why isn't that helpful here?
```{r}
mtcars %>% 
  group_by(cyl) %>% 
  summarise(q = list(quantile(mpg))) %>% 
...
If you can reduce your list column to an atomic vector then it will be a regular column. For example, you can always summarise an object with its type and length, so this code will work regardless of what sort
of list-column you have:
```{r}
df <- tribble(
  ~x,
  letters[1:5],
...
This is the same basic information that you get from the default tbl print method, but now you can use it for filtering. This is a useful technique if you have a heterogeneous list, and want to filter out the parts
...
1.  Why might the [`lengths()`](https://rdrr.io/r/base/lengths.html) function be useful for creating atomic vector columns from list-columns?
...
1.  [`broom::glance(model)`](https://generics.r-lib.org/reference/glance.html) returns a row for each model. Each column gives a model summary: either a measure of model quality, or complexity, or a combination of the two.

2.  [`broom::tidy(model)`](https://generics.r-lib.org/reference/tidy.html) returns a row for each coefficient in the model. Each column gives information about the estimate or its variability.

3.  [`broom::augment(model, data)`](https://generics.r-lib.org/reference/augment.html) returns a row for each row in `data`, adding extra values like residuals, and influence statistics.
